...
int main()
{
	...
	shared_stuffs = (struct struct_shared_mem* )shared_memory;
     	while(flag)
	{
		while(shared_stuffs->write_flag == 1)
		{
			sleep(1);
			printf("Waiting for reader to read.");
		}
		printf("Enter Message : ");
		fgets(msg_buffer, BUFFER_SIZE, stdin);
		strcpy(shared_stuffs->message, msg_buffer);
		printf("Successfully Written : %s || File Name : %s\n", shared_stuffs->message, __FILE__);
		shared_stuffs->write_flag = 1;
